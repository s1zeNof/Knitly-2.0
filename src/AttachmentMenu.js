...
const AttachmentMenu = ({ isOpen, onClose }) => {
    ...
    const desktopItems = [
        { icon: <PhotoIcon />, label: 'Фото або відео', action: () => alert('Photo'), disabled: false },
        { icon: <FileIcon />, label: 'Файл', action: () => alert('File'), disabled: false },
        { icon: <PollIcon />, label: 'Створити опитування', action: () => alert('Poll'), disabled: false },
        { icon: <WalletIcon />, label: 'Гаманець', action: () => {}, disabled: true },
        { icon: <LocationIcon />, label: 'Розташування', action: () => {}, disabled: true },
    ];

    const mobileItems = [
        { icon: <PhotoIcon />, label: 'Галерея', action: () => alert('Photo'), disabled: false, color: 'blue' },
        { icon: <WalletIcon />, label: 'Гаманець', action: () => {}, disabled: true, color: 'cyan' },
        { icon: <FileIcon />, label: 'Файл', action: () => alert('File'), disabled: false, color: 'sky' },
        { icon: <LocationIcon />, label: 'Розташув.', action: () => {}, disabled: true, color: 'green' },
        { icon: <PollIcon />, label: 'Опитування', action: () => alert('Poll'), disabled: false, color: 'yellow' },
        { icon: <MusicIcon />, label: 'Музика', action: () => alert('Music'), disabled: false, color: 'red' },
    ];
