...
const TYPE_META = {
    new_follower:  { color: '#3b82f6', Icon: IconPerson },
    post_like:     { color: '#ef4444', Icon: IconHeart },
    post_comment:  { color: '#22c55e', Icon: IconComment },
    track_like:    { color: '#a855f7', Icon: IconMusic },
    comment_like:  { color: '#f97316', Icon: IconHeart },
    mention_post:  { color: '#f59e0b', Icon: IconAt },
};
...
const NotificationItem = ({ notification, onRead }) => {
    const { fromUser, type, entityLink, entityTitle, entityThumbnail, timestamp, read } = notification;
    if (!fromUser) return null;

    const meta = TYPE_META[type] || { color: '#6b7280', Icon: IconHeart };
    const { Icon } = meta;
    ...
    return (
        <Link
            to={entityLink || `/user/${fromUser.nickname}`}
